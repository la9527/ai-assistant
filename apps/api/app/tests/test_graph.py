...
class ExternalLLMConfigTests(unittest.TestCase):
    ...
    def setUp(self):
        # Runtime .env 값과 무관하게 기본 설정 계층을 검증한다.
        self.default_settings = Settings(_env_file=None)

    def test_external_llm_disabled_by_default(self):
        self.assertFalse(self.default_settings.external_llm.enabled)

    def test_external_llm_default_provider(self):
        self.assertEqual(self.default_settings.external_llm.provider, "openai")

    def test_external_llm_fallback_only_default(self):
        self.assertTrue(self.default_settings.external_llm.fallback_only)
...
class ExternalLLMReplyTests(unittest.TestCase):
    ...
    def test_external_reply_returns_fallback_when_disabled(self):
        ...
        with patch.object(settings, "external_llm_enabled", False), patch.object(settings, "external_llm_api_key", ""):
