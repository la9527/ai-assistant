...
- 시작 줄은 기본적으로 `📬 최근 메일 요약` 의미를 유지하되, 검색 조건이나 날짜 그룹화가 있는 경우 `📬 메일 목록`으로 바꿀 수 있다.
- 검색 조건이 있으면 제목 아래에 `조건: ...` 한 줄을 추가한다.
- 날짜 그룹화가 켜져 있으면 `오늘`, `어제`, `2026-03-18 수요일` 같은 헤더를 사용한다.
- 각 메일은 아래 구조를 따른다.
...
- 날짜 그룹화가 켜진 경우에는 `날짜:` 대신 `시각:` 으로 축약할 수 있다.
- 상태 정보가 있으면 제목 줄 끝에 `(안읽음, 첨부)` 같은 괄호 라벨을 붙인다.
