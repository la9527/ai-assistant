...
- 접근 대상:
  - Open WebUI: `https://<tailscale-host>/`
  - Guacamole: `https://<tailscale-host>/guacamole/`
...
현재 tailnet 내부 브라우저 접근은 `tailscale serve` 를 통해 `https://<tailscale-host>/` 로 제공하는 구성을 권장한다. 이때 Docker proxy는 호스트 `80` 포트만 열고, `443` TLS 종단은 Tailscale Serve가 맡는다.
...
브라우저에서 `https://<tailscale-host>/` 로 접근하려면 Tailscale Serve를 한 번 켜야 한다.
...
- 같은 맥미니에서 `curl https://<tailscale-host>/` 로 self-test 했을 때는 연결되지 않을 수 있다.
- Tailscale Serve는 host `443` 에 일반 서버 프로세스를 띄우는 방식이 아니라 tailnet 경로에서 TLS를 종단하는 방식이므로, 최종 확인은 다른 tailnet 디바이스 브라우저에서 `https://<tailscale-host>/` 로 접속해 보는 것이 더 정확하다.
...
4. `infra/scripts/start-tailscale-serve.sh` 실행 후 브라우저에서 `https://<tailscale-host>/` 접속을 확인한다.
5. 필요하면 `http://<tailscale-host>:5678` 로 n8n 접속을 확인한다.
...
- 예전에는 Tailscale DNS 이름으로 `https://<tailscale-host>/` 접근 시 실패했는데, 원인은 Tailscale Serve 미설정과 Docker의 호스트 `443` 바인딩 충돌이었다.
- 현재 권장 구성은 Docker proxy는 `80` 만 바인딩하고, tailnet HTTPS 종단은 Tailscale Serve가 담당하는 방식이다.
...
- Tailscale DNS 이름으로 브라우저 접근할 때는 `https://<tailscale-host>/` 를 우선 사용하고, 이를 위해 `tailscale serve` 가 활성화돼 있어야 한다.
- Docker proxy는 호스트 `443` 포트를 직접 점유하지 않게 유지한다. 그렇지 않으면 Tailscale Serve의 tailnet HTTPS와 충돌할 수 있다.
